...
int interfaz:main()
{
    ...
    for (int i = 0; i < nVecesSimulacion; i++) //Veces que hace la simulacion
    {
        simulacion cSimulacion; //Variable objeto de la clase

        //Inicia los relojes
        cSimulacion.E1 = 0;
        cSimulacion.E2 = INT_MAX;
        cSimulacion.E3 = INT_MAX;

        //Inicia la simulacion
        while (cSimulacion.Reloj <= tiempoTotal)
        {
            int evento = 0;
            if ((cSimulacion.E1 >= cSimulacion.E2) && (cSimulacion.E1 >= cSimulacion.E3)) //Si el evento 1 debe ocurrir
            {
                cSimulacion.E1();
                evento = 1;
            }
            ...
            {
                if ((cSimulacion.E2 >= cSimulacion.E1) && (cSimulacion.E2 >= cSimulacion.E3))//Si el evento 2 debe ocurrir
                {
                    cSimulacion.E2();
                    evento = 2;
                }
            ...
            vistaSimulacion(cSimulacion.Reloj, evento, cSimulacion.servidorOcupado, cSimulacion.unidadESOcupado,
